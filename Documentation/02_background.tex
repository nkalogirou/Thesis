...
After providing a general introduction to \acp{LLM} and their use of the Transformer architecture, the chapter is intended to clearly distinguish \ac{RAG} as a paradigm for developing large-scale \ac{NLP} applications from several other paradigms which include prompt engineering, fine-tuning and training from scratch. After establishing the primary \ac{RAG} application flow, the chapter covers the different methods for retrieval, reranking, hybrid retrieval and \ac{HPC} concepts relative to this project. Finally, the chapter defines three categories of metrics that will be discussed later in the experimental evaluation: (i) System Performance; (ii) Retrieval Quality; (iii) Answer Quality; (iv) Local vs. HPC Timing Comparison.
...
The \ac{RAG} workflow includes two high-level stages; (i) An indexing stage and (ii) a query-time stage. During the indexing stage, the documents are loaded, cleaned (noise reduction), broken down into manageable pieces (chunks) of data and then embedded using a embedding model for use at query-time. The embedded chunks are then placed in a retrievable structure (e.g., in memory document store or vector database). At query-time, the user question (prompt) is sent through the retrieval process where relevant pieces of data are selected and returned to the system. The selected data is then appended to the prompt and sent to the \ac{LLM} which will generate the final answer.
...
\begin{itemize}
    ...
    \item \textbf{Compute nodes:} The nodes on which the actual workload is executed. Some compute nodes are CPU-only, while others include one or more \acp{GPU} or other accelerators.
    ...
    \item \textbf{Software environments:} \ac{HPC} clusters provide mechanisms for reproducing software environments, including module systems, virtual environments, and, in some systems, containers such as Apptainer.
\end{itemize}
...
\begin{itemize}
    ...
    \item {\textbf{ROUGE-L F1}}:\quad A lexical-overlap-based score (using \ac{ROUGE-L}) compares the generated answer against the ground-truth answer \cite{lin2004rouge}.
