...
The approach aims to use the same collections of documents, questions, open-source language and embedding models, as well as the same format for logging as much as possible throughout the thesis. This enables to measure the influence of different retrieval methods and deployment strategies independently. A baseline of behaviour using the developed notebooks during the local phase, is also established. During the \ac{HPC} phase, the same files and scenarios are run to compare the results of the local and HPC environments within a common evaluation framework.
...
\begin{itemize}
    ...
    \item {\bfseries RQ4: }
    \textbf{How does the same \ac{RAG} workflow behave when moved from local deployment to \ac{HPC} deployment?}\newline
    {\bfseries H4: } 
    When utilizing the exact same files, scenarios, models, and logging formats, there should be minimal differences in query retrieval and response-quality between Local execution and \acf{HPC} as long as they utilize the same resources. The main areas where differences can be seen will be in terms of Performance and Operationally. \ac{HPC} is expected to see a boost in throughput and batched execution through the use of parallelism and batching; however, single-query latency may not consistently have benefits due to scheduling-delays, container startup-times, shared storage access times, data transfer overhead, etc., that will likely offset the added compute power available.
\end{itemize}

These hypotheses are deliberately comparative and quantifiable. 
Each can be evaluated based on the established evaluation framework: retrieval-quality metrics, answer-quality rubrics, timing fields, and environment labels for local and \ac{HPC} execution.
