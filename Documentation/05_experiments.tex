...
The \ac{HPC} experiments were run on the Cyclone \ac{HPC} facility. The service launch scripts submit separate Slurm jobs for the generation model, the embedding model, and the reranking model. Each service job requests 1--node, 1--GPU, 1--task-per-node, and 10--CPU cores.
...
The purpose of this stage is to compare application-level timing behaviour across environments. Queue time, service startup time, and data-transfer time are deployment issues worth considering, but they won’t be treated as separate measured variables in this thesis. The focus is on timing comparisons of indexing time, retrieval time, generation time, and total runtime.
...
\begin{enumerate}
    \item \textbf{\textit{What is the first-response SLA for a Sev-1 support incident?}}\newline This is the clean baseline scenario. It checks whether the system can retrieve and answer a single operational fact accurately.
        \begin{itemize}
            \item Retrieval type: single-document
            \item Main test: exact fact retrieval
        \end{itemize}
    \item \textbf{\textit{What steps must be completed before a new employee is considered ready for day one at OrchestrAI?}}\newline
    This scenario is the broadest scenario because it requires the system to connect support, product, logistics, and commercial-risk information into one operational response.
        \begin{itemize}
        ...
        \end{itemize}
     \item \textbf{\textit{A new engineer starts on Monday, but their laptop has not arrived and their required access is still pending. Which teams are involved and what should happen next?}}\newline
     This scenario requires combining information from more than one team to answer a realistic workplace problem.
        ...
        \end{itemize}
     \item \textbf{\textit{Can OrchestrAI make an urgent hardware purchase without following the normal PO workflow if a warehouse outage is at risk?}}\newline
     This is the most demanding scenario in the notebook because it requires cross-functional reasoning across several business areas.
        ...
        \end{itemize}
     \item \textbf{\textit{A customer renewal is at risk because support cases are recurring, a requested feature is still unavailable, and replacement hardware is delayed. What should OrchestrAI do next, and which teams must be involved?}}\newline
...
Since both environments utilize different model-serving back-ends and some different back-end models, when comparisons are made across environments they should be viewed as a comparison of workflow execution times rather than a hardware comparison. Data generated in this way is then analyzed in chapter ~6