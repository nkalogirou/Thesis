...
The local process utilizes Ollama for model serving. The local generative model utilized is \texttt{llma3.1:8b} and the local embeddings model utilized is \texttt{mxbai-embed-large}. Both of these models are open-source and may be downloaded via Ollama or model repository sources (e.g., Hugging Face) subject to download restrictions on each model. Each notebook that runs reranked vectors locally utilizing \ac{MiniLM} cross encoder (sentence-transformers).
...
The experimental configuration and process of executing all six RAG-pipeline configurations were discussed within this chapter. In addition to the environments used to develop the applications, it was specified that the model-serving environment, input corpuses, parameters, notebooks executed in sequence, and deployment phases would be identical. Additionally, the types of question used to determine the relevance of the retrieved answers, methods used to collect relevant information from each experiment, and steps taken to validate the results of these experiments will be provided.

The local phase will provide an appropriate baseline for comparing the performance of the various retrieval strategies. The \ac{HPC} phase will utilize the exact same pipeline structure and scenario as the local Phase but execute on the Cyclone cluster with \ac{vLLM}-based service utilization. As such, the time comparisons across the two phases are considered to be at the workflow level (i.e., the total execution times across both phases) rather than strictly at the hardware Level. These comparisons will then be evaluated in Chapter~6.