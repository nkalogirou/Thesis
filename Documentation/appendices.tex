...
This appendix records the manual scoring logic used alongside the deterministic evaluation script. The manual assessment was necessary because automatic similarity metrics such as \acs{ROUGE-L}, \acs{BERTScore}, and sentence-embedding similarity can identify overlap with a reference answer, but they do not always determine whether an answer contains the operational detail required by the scenario.
...
\begin{itemize}
    \item \textbf{Answer quality}: manual score, \acs{ROUGE-L} F1, \acs{BERTScore} F1, sentence-embedding semantic similarity, and answer length.
